...
export class SubscriptionMSSQLRepository implements SubscriptionRepository {
    ...
    public async find(id: number): Promise<Subscription | null> {
        ...
        if(result.rowsAffected) return result.recordset[0] as Subscription;
    ...
    public async findByUserAndCode(userId: number, code: string): Promise<Subscription | null> {
        const pool = await connector;
        const result = await pool.query`SELECT * FROM wallet_subscription
    ...
    public async store(entry: Subscription): Promise<void>{
        ...
        await pool.query`INSERT INTO wallet_subscription(user_id, code, amount, cron, created_at)
    ...
    public async update(entry: Subscription): Promise<void> {
        ...
        await pool.query`UPDATE wallet_subscription
                        SET user_id = ${entry.user_id}, code = ${entry.code},
                            amount = ${entry.amount}, cron = ${entry.cron}, updated_at = ${now}
                        WHERE id = ${entry.id}`
    }
